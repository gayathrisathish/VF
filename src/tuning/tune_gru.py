"""
Optuna hyperparameter tuning script — GRU
...
1. Run this once per horizon. Change HORIZON below to "1day", "5day", or "22day"
   and re-run — that's 3 separate runs total for GRU.
2. Run: python src/tuning/tune_gru.py   (from project root)
3. Best hyperparameters get saved to results/best_params/best_params_gru_<horizon>.json
...
GRU trials are already far more expensive than tree-model trials.
