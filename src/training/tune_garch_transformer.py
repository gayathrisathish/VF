"""
Optuna hyperparameter tuning script — GARCH-Transformer (hybrid)
...
ONLY DIFFERENCE FROM tune_transformer.py: the GARCH conditional volatility series
...
the standalone Transformer script, per your finalized spec (item 4): hybrids
reuse the same neural search space, no additional tunable hyperparameters.

HOW TO USE:
1. Run this once per horizon. Change HORIZON below to "1day", "5day", or "22day"
   and re-run — that's 3 separate runs total.
2. Run: python src/tuning/tune_garch_transformer.py   (from project root)
3. Best hyperparameters get saved to results/best_params/best_params_garch_transformer_<horizon>.json
...
SEQUENCE_LENGTH = 30   # fixed — see rationale in standalone Transformer script
...
STUDY_NAME = f"garch_transformer_{HORIZON}"
